...
def extract_from_rxn_smiles(rxn_smiles: str, radius = 0):
    ...
    # """
    # Remove the reagents that do not appear in the products
    # """
    # if sum([mol.GetNumAtoms() for mol in reactants]) !=  sum([mol.GetNumAtoms() for mol in products]):
    #     products_mapnum = [atom.GetAtomMapNum() for atom in match_.GetAtoms()]
    #     reactants_copy = reactants[:]
    #     for mol in reactants_copy:
    #         for atom in mol.GetAtoms():
    #             if atom.GetAtomMapNum() in products_mapnum:
    #                 break
    #         else:
